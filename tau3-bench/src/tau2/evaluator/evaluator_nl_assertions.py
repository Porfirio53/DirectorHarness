...
_nl_assertion_llm = DEFAULT_LLM_NL_ASSERTIONS
_nl_assertion_llm_args = deepcopy(DEFAULT_LLM_NL_ASSERTIONS_ARGS)


def set_nl_assertion_llm_config(model: str, llm_args: dict) -> None:
    """Configure the LLM used by NL assertion evaluators for this process."""
    if not model:
        raise ValueError("NL assertion LLM model must not be empty")
    if not isinstance(llm_args, dict):
        raise TypeError("NL assertion LLM arguments must be a dictionary")

    global _nl_assertion_llm, _nl_assertion_llm_args
    _nl_assertion_llm = model
    _nl_assertion_llm_args = deepcopy(llm_args)
...
class NLAssertionsEvaluator(EvaluatorBase[Message]):
    ...
    def evaluate_nl_assertions(
        ...
        assistant_message = generate(
            model=_nl_assertion_llm,
            messages=messages,
            call_name="nl_assertions_eval",
            **_nl_assertion_llm_args,
        )
