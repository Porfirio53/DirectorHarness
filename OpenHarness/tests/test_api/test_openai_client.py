...
class TestStreamMessageTokenParams:
    @pytest.mark.asyncio
    async def test_stream_forwards_provider_extra_body(self):
        client = OpenAICompatibleClient(
            api_key="test-key",
        ...
        request = ApiMessageRequest(
            model="qwen3.6-plus",
            messages=[ConversationMessage.from_user_text("Use a tool")],
        )
        ...
        assert fake_sdk.chat.completions.last_kwargs["extra_body"] == {"enable_thinking": False}

    @pytest.mark.asyncio
    async def test_request_extra_body_overrides_client_default(self):
        client = OpenAICompatibleClient(
            api_key="test-key",
            extra_body={"enable_thinking": True, "trace": "client"},
        )
        fake_sdk = _FakeOpenAIClient()
        client._client = fake_sdk
        ...
        request = ApiMessageRequest(
            model="qwen3.6-plus",
            messages=[ConversationMessage.from_user_text("Review a plan")],
            extra_body={"enable_thinking": False},
        )
        ...
        assert events
        assert fake_sdk.chat.completions.last_kwargs is not None
        assert fake_sdk.chat.completions.last_kwargs["extra_body"] == {
            "enable_thinking": False,
            "trace": "client",
        }

    @pytest.mark.asyncio
    async def test_stream_forwards_reproducibility_parameters(self):
        client = OpenAICompatibleClient(
            api_key="test-key",
            temperature=0.0,
            seed=42,
        ...
        assert events
        assert fake_sdk.chat.completions.last_kwargs is not None
        assert fake_sdk.chat.completions.last_kwargs["temperature"] == 0.0
        assert fake_sdk.chat.completions.last_kwargs["seed"] == 42
