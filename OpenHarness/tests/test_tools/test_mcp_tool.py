...
class TestInputModelFromSchema:
    ...
    def test_json_encoded_container_strings_are_normalized(self):
        schema = {
            "type": "object",
            "properties": {
                "tags": {"type": "array"},
        ...
        Model = _input_model_from_schema("publish", schema)

        value = Model.model_validate(
            {"tags": '["one", "two"]', "metadata": '{"draft": false}'}
        )
    ...
    def test_invalid_json_container_string_still_fails_validation(self):
        schema = {
            "type": "object",
        ...
        with pytest.raises(ValidationError):
            Model.model_validate({"tags": "not-json"})
