...
@pytest.mark.asyncio
async def test_close_unwinds_multiple_transport_stacks_in_reverse_order():
    manager = McpClientManager({})
    close_order: list[str] = []
    first = MagicMock()
    first.aclose = AsyncMock(side_effect=lambda: close_order.append("first"))
    second = MagicMock()
    second.aclose = AsyncMock(side_effect=lambda: close_order.append("second"))
    manager._stacks = {"first": first, "second": second}
