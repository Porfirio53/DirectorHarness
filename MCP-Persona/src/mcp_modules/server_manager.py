...
def load_key_profiles(path: str):
    """从 JSON 文件读取 key/profile 列表。"""
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)

    key_profiles = []
    for item in data:
        if "key" in item and "profile" in item:
            key_profiles.append({
                "key": item["key"].strip(),
                "profile": item["profile"].strip()
            })
        else:
            print(f"⚠️ Skipped invalid item: {item}")
    return key_profiles
...
class MultiServerManager:
    ...
    def __init__(self, server_configs: List[Dict[str, Any]],
        ...
        self.key_profiles = load_key_profiles("smithery_key.json")
    ...
    async def _connect_stdio_server(self, server_name: str) -> Dict[str, Any]:
        ...
        # 对于 Smithery 服务器，使用 key/profile 重试机制
        for i, creds in enumerate(self.key_profiles, start=1):
            new_params = update_server_params(
                connector.server_params,
                creds["key"],
                creds["profile"]
            )
            # print("new............", new_params)
            connector.server_params = new_params

            print(
                f"🔑 Attempt {i}: Connecting to {server_name} "
                f"with key={creds['key'][:8]}... profile={creds['profile']}"
            )

            try:
                result = await asyncio.wait_for(full_call(), timeout=timeout)
                print(f"✅ Connected successfully with key/profile pair #{i}")
                return result

            except asyncio.TimeoutError:
                print(f"⚠️ Timeout using key/profile pair #{i}, trying next...")
                continue

            except Exception as e:
                print(f"❌ Error connecting with key/profile pair #{i}: {e}")
                continue
        
        # 所有 key/profile 组合都失败，返回空字典
        print(f"❌ All key/profile combinations failed for {server_name}")
        return {}
        # try:
        #     # 对整个 full_call 加统一超时
        #     result = await asyncio.wait_for(full_call(), timeout=timeout)
    ...
    async def _call_tool_stdio(self, connector: MCPConnector, tool_name: str, parameters: Dict[str, Any]) -> Any:
        ...
        # Smithery CLI：执行 key/profile 轮换重试
        for i, creds in enumerate(self.key_profiles, start=1):
            # 更新 server 参数，仅当为 smithery CLI
            new_params = update_server_params(
                connector.server_params,
                creds["key"],
                creds["profile"]
            ...
            print(
                f"🔑 Attempt {i}: Calling tool '{tool_name}' "
                f"with key={creds['key'][:8]}... profile={creds['profile']}"
            )

            try:
                result = await asyncio.wait_for(full_call(), timeout=timeout)
                print(f"✅ Tool call succeeded with key/profile pair #{i}")
                ...
                print(f"❌ Error calling tool with key/profile pair #{i}: {e}")
                continue

        # 所有 key/profile 均失败
        raise RuntimeError("All key/profile combinations failed for tool call.")
